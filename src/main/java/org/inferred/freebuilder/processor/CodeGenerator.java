  ...
  void writeBuilderSource(SourceBuilder code, Metadata metadata, SourceLevel sourceLevel) {
    ...
    switch (metadata.standardMethodUnderride(StandardMethod.EQUALS)) {
      case ABSENT:
        ...
        } else if (sourceLevel.javaUtilObjects().isPresent()) {
          String prefix = "      return ";
          for (Property property : metadata.getProperties()) {
            code.add(prefix);
            code.add("%1$s.equals(%2$s, other.%2$s)",
                sourceLevel.javaUtilObjects().get(), property.getName());
            prefix = "\n          && ";
          }
          code.add(";\n");
        } else {
    ...
    if (metadata.standardMethodUnderride(StandardMethod.HASH_CODE) == ABSENT) {
      ...
      if (sourceLevel.javaUtilObjects().isPresent()) {
        code.addLine("      return %s.hash(%s);", sourceLevel.javaUtilObjects().get(), properties);
      } else {
      ...
      if (metadata.getProperties().isEmpty()) {
        ...
        if (hasRequiredProperties) {
          ...
          code.add("%1$s.equals(_unsetProperties, other._unsetProperties)",
              sourceLevel.javaUtilObjects().get());
        }
        code.add(";\n");
      } else {
        for (Property property : metadata.getProperties()) {
